...
class LiteMono(nn.Module):
    ...
    def forward_features(self, x):
        features = []
        x = (x - 0.45) / 0.225
        x = nn.functional.interpolate(x, scale_factor=(2, 1), mode='bilinear', align_corners=False)
        x_down = []
        for i in range(4):
        ...
        for i in range(0, 3):
            features[i]=nn.functional.interpolate(features[i], scale_factor=(0.5, 1), mode='bilinear', align_corners=False)
        return features
